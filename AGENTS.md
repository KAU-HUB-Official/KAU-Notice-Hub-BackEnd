...
이 프로젝트는 KAU Notice Hub의 FastAPI 백엔드다. 크롤러가 전체 공지 스냅샷 JSON을 주기적으로 atomic 게시하고, 그 직후 `app/ingest.py`가 동일 데이터를 SQLite DB(`NOTICE_DB_PATH`)에 atomic하게 반영한다. API는 SQLite를 읽어 응답한다. DB 파일이 없으면 JSON에서 자동 부트스트랩하고, JSON도 없으면 `JsonNoticeRepository`로 폴백한다. 운영 배포는 Docker Compose, Caddy, GitHub Actions, Lightsail을 기준으로 한다.
...
| `data/`                                    | 런타임 JSON 스냅샷과 SQLite DB 저장 디렉토리 (커밋 금지)            |
...
| `notes/`                                   | 학습/리뷰용 로컬 HTML 노트 (gitignore된 `*.html`)                   |
...
3. 기존 구조에 맞춰 **요청한 내용만 가장 좁은 범위로** 구현한다. 요청 범위 밖의 코드/문구는 임의로 바꾸지 않는다. 다만 최소 범위만 적용하면 기술 부채가 생기거나 구조적 문제가 발견되는 경우에는, 임의로 확장하지 말고 그 내용을 먼저 사용자에게 전달해 수정 여부를 사용자가 결정하게 한다.
...
- 현재 FastAPI, Pydantic, SQLite repository(+ JSON 안전망), service layer 구조를 유지한다.
...
- `/api/chat` RAG 흐름(키워드 추출, 도메인 가드, history multi-turn, SSE 스트리밍, fallback 정책)은 `docs/RAG_PLAN.md`와 `docs/API_SPEC.md`를 따른다. LLM 프롬프트나 가드 분기를 바꾸면 실제 OpenAI 호출 QA로 회귀를 검증한다.
- 사용자가 명시적으로 요구하지 않는 한 Redis, Celery, 외부 DB(Postgres/MySQL 등), 별도 worker 구조를 추가하지 않는다. 운영 저장소는 로컬 SQLite 파일이다. 챗봇 대화 상태는 클라이언트의 `history` 필드로 받고 서버는 stateless를 유지한다.
...
`main`은 배포 브랜치이자 기본 작업 브랜치다. **별도 작업 브랜치를 분기하지 않고 `main`에서 직접 작업하고 커밋한다.** feature 브랜치나 PR 흐름은 사용하지 않는다.

기본 흐름:

1. `main`에서 직접 수정한다. 새 브랜치를 분기하지 않는다.
2. 코드 변경이면 로컬에서 `pytest`를 먼저 실행한다.
3. `main`에 직접 커밋한다.
4. **push는 사용자가 명시적으로 요청할 때만 수행한다.** 커밋까지는 진행할 수 있으나, push(=자동 배포 트리거)는 사용자가 요청하지 않으면 하지 않는다. "커밋해줘"는 push를 포함하지 않으며, 매번 별도로 push를 요청받아야 한다.
5. push하면 GitHub Actions가 Lightsail에 자동 배포한다.

개인 운영 프로젝트라 PR 리뷰 없이 `main` 직접 커밋을 기본 워크플로로 한다. 강제 push와 history rewrite는 하지 않는다.
...
| 챗봇/RAG 흐름, 키워드 추출, 도메인 가드, history, SSE 스트리밍 | `docs/RAG_PLAN.md`, `docs/API_SPEC.md`     |
...
| 저장 JSON 형태, SQLite 스키마, 논리 엔티티                     | `docs/ERD.md`                              |
