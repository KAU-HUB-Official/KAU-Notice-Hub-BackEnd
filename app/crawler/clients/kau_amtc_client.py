...
class KAUAMTCClient(BaseClient):
    ...
    def build_notice_list_url(self, *, bo_table: str, page: int = 1) -> str:
        params = {"bo_table": bo_table}
        if page > 1:
            params["page"] = page
        return f"{self.notice_list_url}?{urlencode(params)}"

    def fetch_notice_list(self, *, bo_table: str, page: int = 1) -> str | None:
        page_url = self.build_notice_list_url(bo_table=bo_table, page=page)
