...
사용자를 받기 시작하기 전에 아래 5개를 순서대로 확인한다. 전부 확인 작업 기준 30분 내외다.

1. **OpenAI 지출 하드 캡** — 서비스 전용 프로젝트 키 발급 + 대시보드에서 프로젝트 월 예산 캡 설정.
   코드에는 총 지출 상한이 없다. 캡 초과 시 OpenAI가 4xx를 반환하고 챗봇은 로컬 검색 폴백으로
   우아하게 강등되므로(챗 서비스의 4xx 처리), 캡이 있어야 최악의 경우가 "챗 품질 저하"로 끝난다.
2. **`INTERNAL_PROXY_TOKEN` 설정** — 랜덤 문자열 생성(`openssl rand -hex 32`) 후
   서버 `.env`의 `INTERNAL_PROXY_TOKEN`과 Vercel 환경변수 `NOTICE_API_INTERNAL_TOKEN`에
   같은 값을 넣고 api 컨테이너 재기동 + Vercel 재배포. 비워두면 BFF 뒤 모든 사용자가
   레이트리밋 버킷 하나를 공유해서 홍보 당일 429가 쏟아진다.
   확인: 서로 다른 네트워크의 기기 2대로 챗을 연타해 429가 기기별로 따로 걸리는지 본다.
3. **컨테이너 메모리 상한** — compose의 api `mem_limit: 750m`이 반영된 상태로 배포하고,
   서버에서 `free -h`로 swap 2G가 실제 존재하는지 확인한다(위 "메모리 가드" 절 참고).
4. **HTTPS** — `API_DOMAIN`이 비어 있으면 Caddy가 평문 `:80`으로 서비스한다. 도메인을 준비해
   서버 `.env`에 `API_DOMAIN=<도메인>`을 넣고 caddy를 재기동하면 인증서 발급과 80→443
   리다이렉트까지 자동이다. 확인: `curl -I https://<도메인>/health` 가 200.
   프론트(Vercel)의 백엔드 URL도 `https://<도메인>`으로 함께 바꾼다.
5. **챗 면책 문구** — 프론트 챗 UI에 "AI 답변은 부정확할 수 있음" 안내가 보이는지 확인한다.
