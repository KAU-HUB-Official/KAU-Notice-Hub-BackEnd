...
`question`(+ 선택 `filters`)에 더해 `must_include_titles`(상위 10건에 떠야 하는 정답
공지 제목 substring)를 라벨로 쓴다. RAGAS 평가는 라벨이 필요 없으므로 자연어 질문만
모은 전용 셋 [tests/eval/ragas_cases.yml](../tests/eval/ragas_cases.yml)을 쓰고
`question`/`filters`만 사용한다.

> 두 셋을 나눈 이유: 라벨 없는 자연어 질문을 `retrieval_cases.yml`에 섞으면 검색
> runner가 그 케이스를 recall=1.0·MRR=0으로 집계해 검색 지표가 왜곡된다. 답변 품질
> 평가는 키워드형 디버깅 케이스보다 실사용 말투의 다양한 질문이 더 유효하기도 하다.
...
| 영역 | 파일 |
| --- | --- |
| 검색 평가셋(라벨 있음) | `tests/eval/retrieval_cases.yml` |
