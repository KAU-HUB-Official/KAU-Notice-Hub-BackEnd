...
운영 챗봇 로그(`chat_sessions.db`)를 검색 품질 연구에 쓸 때는 **이용자에게 연구 활용을
사전 고지한 뒤의 로그만** 사용한다. 논문에 적는 수집 기간과 분석 구간은 아래 표의 날짜를
기준으로 하고, 새 사건이 생기면 이 표에 이어서 적는다.

| 날짜       | 사건                                                                 | 연구 데이터에 미치는 영향                               |
| ---------- | -------------------------------------------------------------------- | ------------------------------------------------------- |
| 2026-09-04 | 검색 단계 trace(`retrieval_json`) 운영 배포 (`0d49295`, 배포 `7e5023d`) | 이후 로그부터 실패가 어느 단계에서 났는지 분석할 수 있다 |
| 2026-09-11 | 챗봇 안내문과 개인정보처리방침에 연구 활용 고지 (FrontEnd `37d47da`, 16:49 KST 운영 반영) | **수집 시작일.** 반영 시각 이전 로그는 연구에 쓰지 않는다 |
| 2026-09-20 | fallback 안내에서 내부 사정 문구 제거 (`25120d4`)                        | fallback 답변 문구가 바뀐다. fallback 여부 판정에는 영향 없음 |
| 2026-09-21 | 게시일 7일 이내 기존 공지 재확인, 수정 시 갱신 (`d1bc69f`)               | 수정된 공지 본문이 답변 근거에 반영되기 시작한다          |
| 2026-09-22 | 상시공지는 매 수집마다 재확인 (`5730114`)                               | 상시공지 수정이 답변 근거에 반영되기 시작한다             |
| 미정       | 신분 태그를 챗봇 위로 이동 (FrontEnd)                                  | 변경 전과 후의 로그를 나눠서 분석한다                    |

- 고지 시행일은 FrontEnd 변경이 **운영에 반영된 날짜**로 확정한다. 커밋일이나 머지일이
  아니다. 배포가 늦어지면 위 표와 `FrontEnd/src/app/privacy/page.tsx`의 `EFFECTIVE_DATE`를
  함께 고친다.
- 고지 이전에 추출한 로그 덤프는 시행일 이후 로그와 섞지 않는다.
- 논문에 질문 예시를 실을 때는 학번, 성명, 연락처처럼 개인을 알아볼 수 있는 부분을
  지운 뒤 싣는다. 처리방침에 "개인을 식별할 수 없도록 처리한 뒤" 활용한다고 고지했으므로
  선택이 아니라 의무다.
- 화면 구성 변경은 날짜만 적는다. 무엇이 바뀌었는지는 FrontEnd 커밋에서 확인한다.
- 지도교수 확인과 연구윤리위원회(IRB) 절차 필요 여부: **미확인**. 확인하면 결과를 여기에
  적는다.
