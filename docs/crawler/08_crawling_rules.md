...
- 캐시에 있는 URL은 상세 요청하지 않습니다. 단 게시일이 최근 `CRAWLER_RECHECK_DAYS`일(기본 7) 이내인 공지는 수정 여부를 확인하려고 다시 읽습니다(아래 '수정된 공지 반영').
...
- 운영 수집에서 목록에 나온 기존 공지 중 게시일이 최근 `CRAWLER_RECHECK_DAYS`일 이내인 공지는 상세를 다시 읽어 해시를 비교합니다. 이런 공지가 있는 페이지는 신규 일반공지가 없어도 다음 페이지까지 확인합니다.
