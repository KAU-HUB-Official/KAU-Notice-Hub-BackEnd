...
- 캐시에 있는 URL은 상세 요청하지 않습니다. 단 게시일이 최근 `CRAWLER_RECHECK_DAYS`일(기본 7) 이내인 공지와 상시공지는 수정 여부를 확인하려고 다시 읽습니다(아래 '수정된 공지 반영').
...
게시 후 본문을 고친 공지가 URL이 같다는 이유로 갱신되지 않던 문제를 막습니다.

- 상세를 파싱할 때 보강 전 원문(제목·본문·첨부 이름과 주소)의 sha256을 `content_hash`로 저장합니다.
- 운영 수집에서 목록에 나온 기존 공지 중 게시일이 최근 `CRAWLER_RECHECK_DAYS`일 이내인 공지는 상세를 다시 읽어 해시를 비교합니다. 상시공지는 게시일이 오래돼도 자주 고치므로 목록에 나올 때마다(매 수집) 확인합니다. 이런 공지가 있는 페이지는 신규 일반공지가 없어도 다음 페이지까지 확인합니다.
- 해시가 다르면 제목·본문·게시일·본문 이미지·첨부를 새 값으로 바꾸고, 이전 보강 결과(`content_original`, `content_enrichment`)를 지워 본문 보강이 새 원문으로 다시 판단하게 합니다. 출처·분류 배열과 상시공지 여부는 그대로 둡니다. 제목 중복으로 합쳐진 공지는 다른 게시판에서 온 첨부를 남깁니다.
- 해시가 없는 예전 공지는 이번에 읽은 해시만 저장합니다. 다시 읽기에 실패하면 기존 공지를 그대로 둡니다.
- 제목 중복으로 합쳐진 공지의 다른 게시판 URL(`source_meta`)은 다시 읽지 않습니다.
- 연구 수집(`--research`)은 다시 읽지 않습니다.
- 로그: `공지 수정 반영 | 게시판=... | 제목=... | url=...`, 저장 로그의 `재확인=`, `수정반영=` 건수.
