...
현재 구현은 feature flag 기본값을 `false`로 두고, 크롤러 최종 JSON 저장 직전에만 보강 pipeline을 실행한다. 본문 이미지 URL은 상세 HTML에서 `content_assets`로 수집하고, 첨부파일은 이미지/HWP/HWPX만 보강 대상으로 분류한다.
...
| ------------------------------------------ | ------------------------------------------------- |
| content가 비어 있음                        | `""`, `None`                                      |
| content가 시스템 fallback임                | `[이미지 본문] ...`, `본문 정보가 비어 있습니다.` |
