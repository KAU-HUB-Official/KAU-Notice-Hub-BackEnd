...
3. 이미지가 여러 개면 `CONTENT_ENRICHMENT_MAX_ASSETS_PER_NOTICE` 개까지만 처리한다.
4. OpenAI vision으로 이미지 텍스트를 추출한다.
5. LLM으로 공지용 content를 생성한다.
...
운영에서 처음 켤 때는 `CONTENT_ENRICHMENT_MAX_CALLS_PER_RUN`을 낮게 두고 수동 crawl에서 먼저 검증한다.
